...
# 문제 풀이
# 입국심사와 같다.
# 최소로 최소로 자를 수 있는 값을 start, 최대로 자를 수 있는 값을 end로 하여 이분탐색을 하여
# 각 랜선에서 몇개를 잘라낼 수 있는지 확인한다.
# 랜선에서 잘라서 얻을 수 있는 갯수가 n보타 크면 결과를 저장하고 start를 mid + 1로 설정,
# 랜선에서 잘라서 얻을 수 있는 갯수가 n보다 작으면 end를 mid - 1로 설정해준다
...
while start <= end:
    mid = (start + end) // 2
    # 만들 수 있는 랜선 갯수 구하기
    count = sum(map(lambda x: x // mid, lines))
    if count >= n:
        maxLength = mid
        start = mid + 1
    else:

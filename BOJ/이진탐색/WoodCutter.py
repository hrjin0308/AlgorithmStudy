...
# 문제 풀이
# 요거슨 떡볶이 떡
# 절단기 높이의 최소값을 start, 최대값을 end로 두고 이분탐색.
# 자른 나무의 길이가 M보다 크면 절단기 높이 값 저장 후 start = mid + 1
# 자른 나무의 길이가 M보다 작으면 end = mid - 1

import sys

n, m = list(map(int, sys.stdin.readline().split()))
woodHeights = list(map(int, sys.stdin.readline().split()))
...
while start <= end:
    mid = (start + end) // 2
    # mid를 절단기 높이로 봤을 때 잘리는 길이 구하기
    # 방법 1: 시간초과
    # length = sum([max(0, x - mid) for x in woodHeights])

    # 방법 2: 4888ms
    length = sum([(x - mid if x - mid > 0 else 0) for x in woodHeights])

    # 방법 3: 시간초과
    # length = 0
    # for wood in woodHeights:
    #     cut = wood - mid
    #     if cut > 0:
    #         length += cut

...
# 1. 입력받은 숫자를 int로 바꿔주고 배열로 쪼개주기
numbers = list(map(int, input()))
# 2. 뒷부분 시작지점 찾기(mid)
mid = len(numbers) // 2
# 3. 0 ~ (mid - 1) 인덱스 까지의 합과 mid ~ 배열 요소 끝까지 합이 일치하면 LUCKY 출력, 아니면 READY 출력
print("LUCKY" if sum(numbers[:mid]) == sum(numbers[mid:]) else "READY")

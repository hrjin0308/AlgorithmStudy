...
#     answer = []
#     dProgresses = deque(progresses)
#     dSpeeds = deque(speeds)
#
#     while len(dProgresses) > 0:
...
def solution(progresses, speeds):
    ...
    for i in range(len(progresses)):
        ...
        print(f"progress {progress}, progress + (remain * speed) {progress + (remain * speed)}")
        if progress + (remain * speed) < 100:
            if count > 0:
                answer.append(count)
            remain = math.ceil((100 - progress) / speeds[i])
            count = 1
            # for i, value in enumerate(dProgresses):
            #     dProgresses[i] = (remain * dSpeeds[i]) + value
        else:
